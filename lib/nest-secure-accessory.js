...
class NestLockAccessory extends NestDeviceAccessory {
    ...
    setLocked(val, callback) {
        let cmd = {
            traitLabel: 'secure_armed',
            command: {
                type_url: 'type.nestlabs.com/nest.trait.security.SecurityArmTrait.SecurityArmRequest',
                value: {
                    armState: val ? 'SECURITY_ARM_STATE_SL1' : 'SECURITY_ARM_STATE_DISARMED',
                    armActor: {
                        method: 'SECURITY_ACTOR_METHOD_REMOTE_USER_EXPLICIT',
                        originator: { resourceId: this.device.user_id },
                        agent: null
                    },
                    acknowledgedIssuesSet: [{
                        factType: 'SECURITY_DECISION_FACT_TYPE_UNSPECIFIED',
                        originResourceId: { resourceId: this.device.user_id },
                        timestamp: (Date.now() / 1000) + 1000
                    }],
                    locationScope: { resourceId: this.device.user_id }
                }
                /* expiryTime: {
